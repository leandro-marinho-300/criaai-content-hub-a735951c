...
export function buildReel2WizardPrefill(draft: Reel2Draft, brand?: Tables<"brands"> | null) {
  ...
  const idea = draft.central_idea || draft.base_content || draft.trend_term || draft.reference_notes || "Reel 2.0";
  const notes = [
    "Origem: Criar Reel 2.0 — Fase 1.",
    draft.entry_mode ? `Entrada escolhida: ${entryLabel(draft.entry_mode as Reel2EntryMode)}.` : "",
    selectedObjective ? `Objetivo do Reel: ${selectedObjective.title}.` : "",
    selectedType ? `Tipo de Reel: ${selectedType.title}. Estrutura: ${selectedType.structure}.` : "",
    draft.promise ? `Promessa do vídeo: ${draft.promise}.` : "",
    selectedHook ? `Gancho escolhido: ${selectedHook.spoken_hook || selectedHook.on_screen_text}.` : "",
  ...
  return {
    ...
    selected_formats: draft.cover_mode === "custom" ? ["reel", "capa_reel"] : ["reel"],
    theme: idea,
    specific_audience: brand?.audience ?? "",
    audience_problem: brand?.audience_difficulties ?? "",
    main_message: draft.promise || draft.central_idea || "Reel com gancho forte, promessa clara e roteiro por cenas.",
    mandatory_information: draft.promise ? `Promessa obrigatória: ${draft.promise}` : "Criar roteiro com gancho nos primeiros 3 segundos, cenas com função e CTA coerente.",
    call_to_action: brand?.calls_to_action?.[0] ?? "",
    desired_style: selectedType ? `${selectedType.title}: ${selectedType.structure}` : "Reel curto, dinâmico e didático.",
