...
export function buildReel2WizardPrefill(draft: Reel2Draft, brand?: Tables<"brands"> | null) {
  ...
  const notes = [
    ...
    imported?.cover?.visual_prompt ? `Prompt visual da capa: ${imported.cover.visual_prompt}` : "",
    imported?.cover?.safe_area_notes ? `Área segura da capa: ${imported.cover.safe_area_notes}` : "",
    imported?.publication?.caption ? `Legenda da publicação: ${imported.publication.caption}` : "",
    imported?.publication?.hashtags?.length ? `Hashtags: ${imported.publication.hashtags.slice(0, 5).join(" ")}` : "",
