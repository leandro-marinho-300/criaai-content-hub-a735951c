...
import { APPROVAL_LABELS, APPROVAL_STATUSES, CHANNEL_LABELS, CHANNELS, SCHEDULE_STATUSES, STATUS_LABELS, computeIsOverdue, type ScheduleStatus, type ApprovalStatus, type ChannelKind } from "@/lib/calendar";
import { changeStatus, deleteScheduleItem, markPublished, undoPublished, upsertScheduleItem, type ScheduleItemWithRels } from "@/lib/scheduleQueries";
import { FORMAT_LABELS } from "@/lib/promptBuilder";
import { ExternalLink, Trash2, Send, RotateCcw, AlertTriangle } from "lucide-react";

interface Props {
  item: ScheduleItemWithRels | null;
  open: boolean;
  onOpenChange: (v: boolean) => void;
}

export function ScheduleDrawer({ item, open, onOpenChange }: Props) {
  ...
  const overdue = computeIsOverdue(current as ScheduleItemWithRels);
  ...
  const statusMut = useMutation({
    mutationFn: (s: ScheduleStatus) => changeStatus(item, s),
    onSuccess: () => { toast.success("Status atualizado."); invalidate(); },
  });

  const publishMut = useMutation({
    mutationFn: () => markPublished(item, { publishedAt: new Date().toISOString(), url: publishUrl || null, notes: publishNotes || null }),
    onSuccess: () => { toast.success("Marcado como publicado."); invalidate(); setPublishUrl(""); setPublishNotes(""); },
  });

  const undoMut = useMutation({
    mutationFn: () => undoPublished(item),
    onSuccess: () => { toast.success("Publicação revertida."); invalidate(); },
  });

  const delMut = useMutation({
    mutationFn: () => deleteScheduleItem(item.id),
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
        <SheetHeader>
          <SheetTitle>{current.title ?? "Sem título"}</SheetTitle>
          <SheetDescription>{current.brands?.name ?? "Sem marca"}</SheetDescription>
        </SheetHeader>
        ...
        <div className="mt-4 space-y-4">
          ...
          {editing ? (
            <div className="space-y-3">
              <Field label="Título"><Input value={current.title ?? ""} onChange={(e) => setDraft((p) => ({ ...p, title: e.target.value }))} /></Field>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {current.project_id && (
                <Button asChild variant="outline" size="sm">
                  <Link to="/app/content/$projectId/result" params={{ projectId: current.project_id }} onClick={() => onOpenChange(false)}>
              ...
              <Button variant="outline" size="sm" onClick={() => statusMut.mutate("cancelado")} disabled={status === "cancelado"}>Cancelar</Button>
